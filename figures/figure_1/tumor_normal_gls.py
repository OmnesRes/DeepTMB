...
data = pickle.load(open(cwd / 'tables' / 'table_1' / 'DUKE-F1-DX1' / 'data' / 'data.pkl', 'rb'))
...
nci = np.array([nci_dict[i] for i in data if (data[i][0] / (data[i][1] / 1e6)) < cutoff])
...
linear_count_encoder = Encoders.Encoder(shape=(1,), layers=())
linear_net = NN(encoders=[linear_count_encoder.model], layers=(), mode='tfp_linear_regresion')

linear_net.model.compile(loss=utils.log_prob_loss,
                  optimizer=tf.keras.optimizers.Adam(learning_rate=0.001))

fcn_count_encoder = Encoders.Encoder(shape=(1,), layers=())
fcn_net = NN(encoders=[fcn_count_encoder.model], layers=(), mode='fcn')
...
linear_net.model.fit(ds_train_mixture,
              steps_per_epoch=10,
              epochs=10000,
              callbacks=callbacks
              )
...
Z = linear_net.model(x_pred).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
X_pred, Y_pred = np.meshgrid(x_pred, y_pred)

mixture_means = linear_net.model(x_pred).mean()
fcn_means = fcn_net.model.predict(x_pred)
quantiles = [y_pred[np.argmin(np.diff((np.exp(linear_net.model(x_pred).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.05).astype(int), axis=0), axis=0)],
             y_pred[np.argmin(np.diff((np.exp(linear_net.model(x_pred).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.25).astype(int), axis=0), axis=0)],
             y_pred[np.argmin(np.diff((np.exp(linear_net.model(x_pred).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.5).astype(int), axis=0), axis=0)],
             y_pred[np.argmin(np.diff((np.exp(linear_net.model(x_pred).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.75).astype(int), axis=0), axis=0)],
            y_pred[np.argmin(np.diff((np.exp(linear_net.model(x_pred).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.95).astype(int), axis=0), axis=0)]]
medians = y_pred[np.argmin(np.diff((np.exp(linear_net.model(X).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.5).astype(int), axis=0), axis=0)]
...
ax.plot(x_pred, mixture_means, color='k', linestyle='-', alpha=1, label='GLS means')
...
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'normal_gls_input_output.png', dpi=600)
...
Z = linear_net.model(np.array([t.trf(4)])[:, np.newaxis]).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
...
y_pred = np.linspace(min(counts.keys()), max(list(counts.keys())), 200)
Z = linear_net.model(np.array([t.trf(8.5)])[:, np.newaxis]).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
zz = zz / np.sum(zz, axis=0) * (200 / len([i / 10 for i in range(int(min(counts.keys()) * 10), int(max(counts.keys()) * 10) + 1, int(bin * 10))]))
...
y_pred = np.linspace(min(counts.keys()), max(list(counts.keys())), 200)
Z = linear_net.model(np.array([t.trf(17.5)])[:, np.newaxis]).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
zz = zz / np.sum(zz, axis=0) * (200 / len([i / 10 for i in range(int(min(counts.keys()) * 10), int(max(counts.keys()) * 10) + 1, int(bin * 10))]))
...
y_pred = np.linspace(min(counts.keys()), max(list(counts.keys())), 200)
Z = linear_net.model(np.array([t.trf(30)])[:, np.newaxis]).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
zz = zz / np.sum(zz, axis=0) * (200 / len([i / 10 for i in range(int(min(counts.keys()) * 10), int(max(counts.keys()) * 10) + 1, int(bin * 10))]))
...
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'normal_gls_distributions.png', dpi=600)
...
ax.set_ylim(-2, 2)
ax.set_xlim(-.01, t.trf(60))
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'normal_gls_residuals.png', dpi=600)
