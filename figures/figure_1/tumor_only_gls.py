...
ds_train_gls = ds_train.map(lambda x: ((
                                    X_loader(x),
                                    ),
                                   (Y_loader(x),
                                    ),
...
linear_net.model.fit(ds_train_gls,
              steps_per_epoch=10,
              epochs=10000,
              callbacks=callbacks
              )
...
gls_means = linear_net.model(x_pred).mean()
...
ax.plot(x_pred, gls_means, color='k', linestyle='-', alpha=1, label='GLS means')
...
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'germline_gls_input_output.png', dpi=600)
...
Z = linear_net.model(np.array([t.trf(7)])[:, np.newaxis]).log_prob(y_pred[:, np.newaxis]).numpy()
zz = np.exp(Z - Z.max(axis=0, keepdims=True))
...
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'germline_gls_distributions.png', dpi=600)
...
ax.set_ylim(-2, 2)
ax.set_xlim(-.01, t.trf(60))
plt.savefig(cwd / 'figures' / 'figure_1' / 'results' / 'germline_gls_residuals.png', dpi=600)
