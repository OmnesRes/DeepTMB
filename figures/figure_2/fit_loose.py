...
data = pickle.load(open(cwd / 'figures' / 'figure_2' / 'data' / 'data.pkl', 'rb'))
...
cutoff = np.percentile([i[0][0] / (i[1] / 1e6) for i in data.values()], 98)
values = [i for i in data.values() if (i[0][0] / (i[1] / 1e6)) < cutoff]
nci = np.array([nci_dict[i] for i in data if (data[i][0][0] / (data[i][1] / 1e6)) < cutoff])
anc = np.array([ancestry.get(i[:12], 'OA') for i in data if (data[i][0][0] / (data[i][1] / 1e6)) <= cutoff])
anc_encoding = {'AA': 1, 'EA': 2, 'EAA': 3, 'NA': 4, 'OA': 0}
anc = np.array([anc_encoding[i] for i in anc])

X = np.array([i[0][0] / (i[1] / 1e6) for i in values])
...
ax.hlines(t.trf(10), -.1, t.trf(60), linestyles='dotted', color='k', zorder=-1000, alpha=.3)
...
ax.set_ylim(-.1, t.trf(40))
ax.set_xlim(-.1, t.trf(60))
