...
def get_overlap(tumor):
    ...
    for maf in [loose_tumor_only_maf, moderate_tumor_only_maf, strict_tumor_only_maf]:
        ...
        if len(tumor_only_df) == 0:
            return None
        tumor_only_pr = pr.PyRanges(tumor_only_df[['CHROM', 'POS', 'End_Position']].rename(columns={'CHROM': 'Chromosome', 'POS': 'Start', 'End_Position': 'End'}))
        grs = {'bed_panel_cds': bed_panel_cds_pr}
        result = pr.count_overlaps(grs, pr.concat({'maf': tumor_only_pr}.values()))
        result = result.df
        panel_counts.append(sum(result['bed_panel_cds'] > 0))
    return [panel_counts, panel_size, exome_counts, bed_size]
