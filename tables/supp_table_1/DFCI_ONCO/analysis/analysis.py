...
X = np.array([i / (panels.loc[panels['Panel'] == 'DFCI-ONCOPANEL-3']['cds'].values[0] / 1e6) for i in samples['panel_counts'].values])
...
run_predictions, test_idx, values = pickle.load(open(cwd / 'tables' / 'supp_table_1' / 'DFCI_ONCO' / 'results' / 'run_nonsyn_predictions.pkl', 'rb'))
...
run_predictions, test_idx, values = pickle.load(open(cwd / 'tables' / 'supp_table_1' / 'DFCI_ONCO' / 'results' / 'run_cds_predictions.pkl', 'rb'))
run_predictions = np.exp(np.concatenate(run_predictions)) - 1
print(round(np.sqrt(np.mean((run_predictions - Y[np.concatenate(test_idx)])**2)), 2))
print(round(np.mean(np.abs(run_predictions - Y[np.concatenate(test_idx)])), 2))
print(round(r2_score(Y[np.concatenate(test_idx)], run_predictions), 2))
...
run_predictions, test_idx, values = pickle.load(open(cwd / 'tables' / 'supp_table_1' / 'DFCI_ONCO' / 'results' / 'run_predictions.pkl', 'rb'))
run_predictions = np.exp(np.concatenate(run_predictions)) - 1
print(round(np.sqrt(np.mean((run_predictions - Y[np.concatenate(test_idx)])**2)), 2))
print(round(np.mean(np.abs(run_predictions - Y[np.concatenate(test_idx)])), 2))
print(round(r2_score(Y[np.concatenate(test_idx)], run_predictions), 2))
