...
for idx_train, idx_test in StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(anc, anc):
    ...
    losses.append(net.model((X[idx_test], anc[idx_test])).log_prob(Y[idx_test]).numpy())


with open(cwd / 'tables' / 'supp_table_2' / 'DUKE-F1-DX1' / 'tumor_only_strict' / 'results' / 'with_ancestry.pkl', 'wb') as f:
