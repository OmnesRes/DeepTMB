...
predictions = y_pred[np.argmin(np.diff((np.exp(net.model(X).log_cdf(y_pred[:, np.newaxis]).numpy()) < 0.5).astype(int), axis=0), axis=0)]
losses = net.model(X).log_prob(Y).numpy()


with open(cwd / 'tables' / 'supp_table_8' / 'tumor_only_strict' / 'results' / 'without_ancestry.pkl', 'wb') as f:
    pickle.dump([predictions, values, losses], f)
