...
for ancestry in range(1, 6):
    ...
    print('linear prob')
    run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / ('linear_prob_nonsyn_predictions_' + str(ancestry) + '.pkl'), 'rb'))
    ...
    print('gmm prob')
    run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / ('gmm_nonsyn_predictions_' + str(ancestry) + '.pkl'), 'rb'))
    print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
    print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))

    print('gmm hotspot')
    run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / ('gmm_hotspots_predictions_' + str(ancestry) + '.pkl'), 'rb'))
    print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
    print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))

    print('gmm syn')
    run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / ('gmm_syn_predictions_' + str(ancestry) + '.pkl'), 'rb'))
    print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
    print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))

    print('gmm syn hotspots')
    run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / ('gmm_syn_hotspots_predictions_' + str(ancestry) + '.pkl'), 'rb'))
    print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
    print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))

    if ancestry == 5:
        print('gmm syn hotspots ancestry')
        run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / 'gmm_syn_hotspots_ancestry_predictions.pkl', 'rb'))
        print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
        print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))

        print('gmm multi ancestry')
        run_predictions, values, losses = pickle.load(open(cwd / 'tables' / 'supp_tables_1-5' / 'tumor_normal' / 'results' / 'gmm_multi_ancestry_predictions.pkl', 'rb'))
        print(round(np.mean(np.abs(Y_anc[tmb_high] - t.inv(run_predictions[tmb_high]))), 2))
        print(round(spearmanr(t.inv(run_predictions[tmb_high]), Y_anc[tmb_high])[0], 2))
