...
X = np.array([(i[-1] - i[4]) / (i[1] / 1e6) for i in data.values()])
Y = np.array([i[2] / (i[3] / 1e6) for i in data.values()])

t = utils.LogTransform(bias=4, min_x=min(X))
...
for ancestry in range(1, 6):
    ...
    with open(cwd / 'tables' / 'supp_tables_3-7' / 'tumor_only_strict' / 'results' / ('gmm_hotspots_predictions_' + str(ancestry) + '.pkl'), 'wb') as f:
