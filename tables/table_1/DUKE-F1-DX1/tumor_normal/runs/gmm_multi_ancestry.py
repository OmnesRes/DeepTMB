...
X = np.stack([np.array([i[0], i[4], j]) / (i[1] / 1e6) for i, j in zip(data.values(), non_syn_data.values())], axis=0)
...
with open(cwd / 'tables' / 'table_1' / 'DUKE-F1-DX1' / 'tumor_normal' / 'results' / 'gmm_multi_ancestry_predictions.pkl', 'wb') as f:
