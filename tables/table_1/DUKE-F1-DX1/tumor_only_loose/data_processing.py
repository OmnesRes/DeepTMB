...
loose_tumor_only_maf = pickle.load(open(cwd / 'files' / 'germline' / 'data' / 'tumor_only_maf_filtered_loose.pkl', 'rb'))
...
del annot
loose_tumor_only_maf['non_syn'] = loose_tumor_only_maf['ID'].apply(lambda x: annot_dict[x])
loose_tumor_only_maf['non_syn'] = loose_tumor_only_maf['non_syn'] | loose_tumor_only_maf['merged_ID']
...
def get_overlap(tumor):
    ...
    if bed_size < 25000000:
        return None
    tumor_only_df = loose_tumor_only_maf.loc[loose_tumor_only_maf['bcr_patient_barcode'] == tumor[:12]]
...
with open(cwd / 'tables' / 'table_1' / 'DUKE-F1-DX1' / 'tumor_only_loose' / 'data' / 'data.pkl', 'wb') as f:
    pickle.dump(data, f)
