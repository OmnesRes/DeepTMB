...
annot, starting_maf = pickle.load(open(cwd / 'files' / 'germline' / 'data' / 'tcga.genie.combined.annot.maf.pkl', 'rb'))
del starting_maf

strict_tumor_only_maf = pickle.load(open(cwd / 'files' / 'germline' / 'data' / 'tumor_only_maf_filtered_strict.pkl', 'rb'))
non_syn = ['missense', 'frameshift', 'stop_gained', 'inframe_insertion', 'inframe_deletion', 'stop_gained&frameshift', 'stop_lost', 'stop_gained&inframe_deletion', 'stop_lost&frameshift', 'stop_lost&inframe_deletion', 'stop_gained&inframe_insertion']

annot['non_syn'] = annot['INFO/BCSQ_csqs'].str.split('|').apply(lambda x: x[0]).isin(non_syn)
annot_dict = {i: j for i, j in zip(annot['ID'], annot['non_syn'].values)}
del annot
strict_tumor_only_maf['non_syn'] = strict_tumor_only_maf['ID'].apply(lambda x: annot_dict[x])
strict_tumor_only_maf['non_syn'] = strict_tumor_only_maf['non_syn'] | strict_tumor_only_maf['merged_ID']
...
def get_overlap(tumor):
    ...
    if bed_size < 25000000:
        return None
    tumor_only_df = strict_tumor_only_maf.loc[strict_tumor_only_maf['bcr_patient_barcode'] == tumor[:12]]
    if len(tumor_only_df) == 0:
        return None
    tumor_only_pr = pr.PyRanges(tumor_only_df[['CHROM', 'POS', 'End_Position', 'hotspot', 'non_syn']].rename(columns={'CHROM': 'Chromosome', 'POS': 'Start', 'End_Position': 'End'}))
    grs = {'bed_panel_cds': bed_panel_cds_pr}
    result = pr.count_overlaps(grs, pr.concat({'maf': tumor_only_pr}.values()))
    result = result.df
    panel_counts = sum(result['bed_panel_cds'] > 0)
    panel_hotspots = sum((result['bed_panel_cds'] > 0) & result['hotspot'])
    panel_nonsyn = sum((result['bed_panel_cds'] > 0) & result['non_syn'])
    return [panel_counts, panel_size, exome_counts, bed_size, panel_hotspots, panel_nonsyn]
...
with open(cwd / 'tables' / 'table_1' / 'DUKE-F1-DX1' / 'tumor_only_strict' / 'data' / 'data.pkl', 'wb') as f:
    pickle.dump(data, f)
